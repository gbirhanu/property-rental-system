...
export default class DisplayMoreMachinePhoto extends Component {
  state = {
    index: 0
  };
  render() {
    return (
      <ResourceConsumer>
        {value => {
          const { morePhotoOpenMachine } = value;
          const housePhoto = value.filteredPhotoSingelMachine;
          if (!morePhotoOpenMachine) {
            return null;
          } else {
            console.log(value.filteredPhotoSingelMachine);
            return (
              <ModalContainer>
                <div className="close-modal-button">
                  <FontAwesomeIcon
                    icon={faWindowClose}
                    onClick={() => value.closePhotoModalMachine()}
                  />
                </div>
